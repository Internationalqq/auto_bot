...
Покрытие по стоимости, %: {getattr(st, "coverage_cost_percent", None)}
Сумма позиций без цены: {getattr(st, "uncovered_estimate_total", None)}
Позиций без определённой суммы: {getattr(st, "rows_without_amount", 0)}
"""
    prompt = f"""{user_block}
Напиши по-русски для подрядчика, который смотрит на закупку:
1) Одно предложение о полноте проверки и разнице со сметой только по проверенным строкам. Не называй тендер выгодным или невыгодным: цена предложения и все затраты не определены.
