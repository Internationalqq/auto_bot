...
(async () => {
  ...
  assert.deepEqual(posts.at(-1), {action:'prepare_suppliers',position_keys:['cable']});
  ...
  start.events.click(); await flush();
  assert.deepEqual(posts.at(-1), {action:'prepare_suppliers'});
  rows.forEach(r => r.checked = true); counter.textContent = '2'; await flush();
  assert.match(start.textContent, /\(2\)/);
  start.events.click(); await flush();
  assert.deepEqual(posts.at(-1), {action:'prepare_suppliers',position_keys:['cable','work']});
