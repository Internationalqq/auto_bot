...
(async () => {
  ...
  assert.deepEqual(posts.at(-1), {position_keys:['cable']});
  ...
  assert.equal(start.textContent, 'Подготовить обращения');
  start.events.click(); await flush();
  ...
  start.events.click(); await flush();
  assert.deepEqual(posts.at(-1), {position_keys:['cable','work']});
